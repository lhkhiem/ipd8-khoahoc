...
export default function EditEducationResourcePage() {
  ...
  const fetchResource = async () => {
    try {
      setLoading(true);
      const url = buildApiUrl(`/api/homepage/education-resources/${id}`);
      console.log('Fetching education resource from:', url);
      ...
      if (!response.ok) {
        ...
        const errorMessage = errorData.error || errorData.message || `Failed to fetch resource (${response.status} ${response.statusText})`;
      ...
      console.log('Fetched resource:', resource); // Debug log

      // Handle image URL - if it's relative, convert to full URL for display
      const imageUrlValue = resource.image_url || '';
      const displayImageUrl = imageUrlValue?.startsWith('http') 
        ? imageUrlValue 
        : imageUrlValue 
          ? buildBackendUrl(imageUrlValue) 
          : '';

      setTitle(resource.title || '');
      // Set slug - if not exists, generate from title
      const resourceSlug = resource.slug || (resource.title ? generateSlug(resource.title) : '');
      setSlug(resourceSlug);
      setManualEdit(!!resource.slug); // Only set manual edit if slug already exists
      setDescription(resource.description || '');
      setImageUrl(displayImageUrl);
      setImageUrlInput(imageUrlValue); // Keep original value for saving
      setLinkUrl(resource.link_url || '');
      setLinkText(resource.link_text || 'Learn More');
      setDuration(resource.duration || '');
      setCeus(resource.ceus || '');
      setLevel(resource.level || '');
      setResourceType(resource.resource_type || 'course');
      ...
      setSelectedTopicIds((resource.topics || []).map((t: any) => t.id));
      setSelectedTagIds((resource.tags || []).map((t: any) => t.id));
    } catch (error) {
      console.error('Failed to fetch resource:', error);
      const errorMessage = error instanceof Error ? error.message : 'Failed to load resource';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const json = await response.json().catch(() => ({ error: 'Failed to update resource' }));
        throw new Error(json.message || json.error || 'Failed to update resource');
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Edit Education Resource</h1>
        <Link
